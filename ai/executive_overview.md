...
- **Format-agnostic G-buffer** — Shaders access G-buffer images through format-less storage image reads/writes (`shaderStorageImageReadWithoutFormat`). The host can use bandwidth-efficient compact formats (32% savings: RG16F motion, R16F depth, R11G11B10F albedo) or standard RGBA16F for any channel — same shaders, same API, no permutations. Compact formats reduce bandwidth on desktop and are critical on mobile.
