...
4. ~~`training/scripts/validate_dataset.py`~~ — **Removed.** Used a stale 13-ch/3-ch schema that no longer matches the training pipeline (19-ch input / 6-ch target). Dataset validation is handled by unit tests and manual inspection.
...
- ~~`validate_dataset.py`~~ removed (stale schema)
- Visual inspection shows recognizable noisy inputs and clean targets across exposure range
...
6. **Pipeline validation run** — render 2 viewpoints per scene to verify pipeline and multi-viewpoint iteration:
   ...
   - Visually inspect rendered output for each scene
...
8. **Spot-check rendered output** — visually inspect each scene to verify:
...
9. **Run a short training test** (5 epochs) with the full dataset:
...
10. **Update `training/configs/default.yaml`**:
...
- No NaN/Inf issues in rendered output
...
6. **Pipeline validation — render sample light-rig frames:**
   ...
   - Visually inspect the test output
...
7. **Generate supplementary dataset** (production):
   ...
   - Visually inspect the full combined dataset
...
- Combined 2,240-frame dataset has no NaN/Inf, all expected files present
...
| F9-4 | `export_cornell_box.py`, `download_scenes.py`, `generate_training_data.py`, `training/.gitignore` | `requirements.txt` |
...
| F9-6d | Dataset generation orchestration | `generate_training_data.py` |
