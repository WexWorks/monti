...
Viewpoint files are JSON arrays that define camera positions and per-viewpoint rendering parameters for `monti_datagen`. Each entry produces one frame of training data. For temporal training, entries are grouped into paths via `path_id` and ordered by `frame`.
...
|---|---|---|---|
| `path_id` | string | — | 8-hex-char path identifier (shared across all frames in one path; generated by `monti_view` tracking mode) |
| `frame` | int | — | 0-indexed frame number within the path (defines render order for temporal data generation) |
...
| `environmentRotation` | float | 0.0 | Environment map rotation in degrees around the Y axis |
...
**Viewpoint authoring:**
- **Path tracking mode:** In `monti_view`, press `P` to toggle path tracking mode. While enabled, any camera motion automatically starts recording a new path — a new frame is buffered whenever the camera moves more than a minimum threshold. When motion stops for ~500ms the path is flushed to the viewpoints JSON file and a new path begins on the next movement. The currently loaded environment map path, rotation, and intensity are embedded in every captured frame. Press `P` again to disable tracking and discard any in-progress path.
- **Undo:** Press `Backspace` to remove the last flushed path from the viewpoints JSON file.
- **File naming:** The output JSON file is auto-named from the scene (e.g., `DamagedHelmet.glb` → `damaged_helmet.json`) in `training/viewpoints/`.
...
| **P** | **Toggle path tracking mode (start/stop recording camera paths to viewpoints JSON)** |
| **Backspace** | **Delete the last recorded path from the viewpoints JSON file** |
...
- **Path tracking indicator:** green dot + `"Tracking"` when path recording mode is active; `"Capturing [a1b2c3d4] — N frames"` while a path is being buffered
...
│   ├── main.cpp                        # Entry point, CLI parsing, render loop, path tracking mode (P key)
