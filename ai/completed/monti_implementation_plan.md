# Monti & Deni — Completed Implementation Phases

> **Archive:** This file contains the detailed tasks and verification for all completed phases of the Monti path tracer and Deni denoiser implementation. All initial-release phases (1–8N, 9A–9D, 10A–10B, 11A–11B) are complete.
...
- **PackedMaterial `transmission_ext` vec4.** Phase 8H added a `transmission_ext` vec4 to `PackedMaterial` for diffuse transmission data with half-float packing for the color. Subsequent phases (8L, 8M, 8N) added further vec4s; the final struct is 11 vec4s (176 bytes) with `kMaterialStride = 11u`:
  ...
  - `.a` = `packHalf2x16(vec2(diffuse_transmission_color.b, float(nested_priority)))` — blue channel in upper half, `nested_priority` in lower half (packed by Phase 8I ✅)
...
- **MIS update: 5-way strategy with energy-conserving diffuse split.** The diffuse transmission lobe adds a fifth sampling strategy. `SamplingProbabilities` gains a `diffuse_transmission` field. The existing diffuse probability is split by `diffuse_transmission_factor` to maintain energy conservation:
  ...
  **Edge case: `diffuse_transmission_factor = 1.0` with `transmission_factor < 1.0`.** When `diffuse_transmission_factor = 1.0`, all smooth-lobe diffuse energy goes to diffuse transmission and `prob_diffuse_reflect = 0`. This is a valid configuration for a fully translucent, non-specular material (e.g., dense fabric or frosted film) where no diffuse light reflects from the front face — all diffuse energy transmits through. The specular GGX lobe, clearcoat, and environment strategies remain active and provide the only front-hemisphere contributions. Energy is conserved because `prob_diffuse_reflect + prob_diffuse_transmit = base_diffuse` regardless of the split.
...
- **`double_sided` interaction with diffuse transmission.** When the camera hits the back face of a double-sided material, the shading normal `N` is flipped to face the camera. Diffuse transmission then transmits into `-N` (away from the camera), which is the geometric "front" of the surface. This is physically correct for a thin material: regardless of which face you view, light transmits through to the opposite side.
  ...
  **Single-sided thin surfaces:** Currently, all surfaces flip their normals to face the ray regardless of the `double_sided` flag — the `double_sided` flag is parsed and stored in `MaterialDesc` but not packed into `PackedMaterial` or enforced in shaders. Proper single-sided culling (rejecting back-face hits for non-double-sided surfaces) is deferred to a future phase. For thin translucent materials this is acceptable: glTF specifies that `KHR_materials_diffuse_transmission` implies double-sided geometry, and the loader sets `thin_surface = true` (and implicitly double-sided) when the extension is present.

- **cgltf v1.14 does not natively support `KHR_materials_diffuse_transmission`.** The extension must be parsed via cgltf's generic `extensions[]` array on each material. Each unrecognized extension is stored as `{name, data}` where `data` is a raw JSON string. A minimal JSON parser extracts `diffuseTransmissionFactor` (float), `diffuseTransmissionColorFactor` (float[3]), and optional `diffuseTransmissionTexture`. If the extension is present, `thin_surface` is set to `true` (glTF diffuse transmission implies thin-surface geometry).

  **`diffuseTransmissionTexture` is not supported in v1** — the loader logs a warning and ignores the texture reference. Support for `diffuseTransmissionTexture` (a texture that modulates `diffuseTransmissionFactor` per-texel) is tracked as a future enhancement; see roadmap.md.
...
2. Extend `PackedMaterial` with `transmission_ext` vec4:
   ```cpp
   glm::vec4 transmission_ext;  // .r = diffuse_transmission_factor,
                                 // .g = thin_surface (0.0/1.0),
                                 // .b = packHalf2x16(dt_color.rg) as float,
                                 // .a = packHalf2x16(vec2(dt_color.b, float(nested_priority))) as float
   ```
   Note: Phase 8H originally reserved `.a`'s lower half as `0.0`; Phase 8I now packs `nested_priority` there. Final struct is 11 vec4s (176 bytes) after phases 8L/8M/8N added `uv_transform`, `sheen`, and `sheen_textures`.
...
3. Update `GpuScene::UpdateMaterials()` to pack new fields:
   ...
   - Pack `diffuse_transmission_color.b` and `float(nested_priority)` via `glm::packHalf2x16()` → `std::bit_cast<float>()` into `.a` (Phase 8I ✅ now packs `nested_priority` in the lower half)

4. Update `shaders/include/constants.glsl`:
   - `kMaterialStride` updated (was 7 before 8H; now 11u after phases 8H/8L/8M/8N)
...
8. Update glTF loader to parse `KHR_materials_diffuse_transmission`:
   ...
   - Parse the raw JSON `data` string (a simple key-value object) for:
     ...
     - `"diffuseTransmissionTexture"` → not supported in v1; log warning and skip (see roadmap.md for future enhancement)
