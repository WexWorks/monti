...
//
// SYNC REQUIREMENT: The golden reference file (tests/data/golden_ref_v3.bin)
// is generated by tests/generate_golden_reference.py, which imports the
// DeniTemporalResidualNet model from training/deni_train/models/. If the
// training model architecture changes, the golden reference must be
// regenerated:
//
//   cd training
...
std::optional<GoldenReference> LoadGoldenReference(const std::string& path) {
    ...
    uint32_t version = 0;
    if (!read_u32(version) || version != 2) return std::nullopt;
